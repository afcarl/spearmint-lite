...
Google Protocol Buffers (for the fully automated code)
...
well suited to the Amazon EC2 system.  Using StarCluster
(http://star.mit.edu/cluster/) will allow you to set up a large
...
name specified.  Now take a look at branin.py (the wrapper which was
...
'python spearmint_sync.py --method=GPEIOptChooser
--method-args=noiseless=1 braninpy'. This will run spearmint according
to the GP-EI MCMC strategy.  The code will sequentially spawn
...
job-id.txt in the output directory.  If you are debugging your code,
